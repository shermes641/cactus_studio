...
export const handler: Handler = async (event: any, context: any) => {
  ...
  try {
    ...
    if (token) {
        const rows = await sql`SELECT id, email, name, phone, shipping_addr, cart, is_admin, discount_code FROM users WHERE session_token = ${token}`;
    ...
    if (email) {
        const rows = await sql`SELECT id, email, name, phone, shipping_addr, cart, is_admin, discount_code FROM users WHERE email = ${email}`;
